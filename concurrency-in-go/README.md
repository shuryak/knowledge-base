...
Когда люди обсуждают Go, часто можно услышать аббревиатуру *CSP*. Часто CSP
превозносят как причину успеха Go или панацею для конкурентного
программирования. Этого достаточно, чтобы люди, которые не знают, что такое
CSP на самом деле, начали думать, что в Computer Science открыли какую-то новую
...
что называется *исчислением процессов*. Исчисление процессов — это способ
...
Это достигается благодаря гарантии, которую даёт Go: горутины легковесны, и
нам обычно не придётся беспокоиться об их создании. Существуют подходящие
моменты, чтобы обратить внимание на количество работающих в системе горутин, но
делать это заранее — ненужная оптимизация, в отличие от потоков, где было бы
разумно обдумать такие вопросы заранее.
...
конкурентных проблем не имеет значения. Фреймворк написан кем-то и наш код
всё равно под собой будет иметь те сложности, с которыми авторам фреймворка
пришлось столкнуться. Тот факт, что сложность скрыта от нас, не значит, что её
нет, а сложность порождает ошибки и баги. В случае Go, язык был разработан с
учётом конкурентности, поэтому язык не противоречит предоставляемым им
примитивам конкурентности. Это значит меньше разногласий и меньше багов.
...
>
> (*В Go 1.5 порядок, в котором горутины планируются, был изменён.*)
...
И горутины — это только одна часть пазла. Другие концепции CSP, такие как
каналы и выражения `select` также вносят свой вклад.
...
случайным образом, продолжать работу, если нет процессов, ожидающих сообщений,
и т.д.
...
>
> **Примечание**: *Мьютекс — это другое название взаимоисключающей блокировки*.
...
- *Вы собираетесь передать владение данными?*
...
- *Вы собираетесь защитить внутреннее состояние структуры?*
  ...
  Небольшой пример типа, который является потокобезопасным, но не раскрывает
  эту сложность вызывающим объектам:
  ...
  Следует помнить, что ключевое слово здесь — *внутреннее*. Если мы ловим себя
...
- *Вы собираетесь согласовать несколько частей логики?*
...
- *Это критически важная для производительности секция?*
